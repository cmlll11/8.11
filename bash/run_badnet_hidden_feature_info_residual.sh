...
# Analyze the four separate x+f(x) generators; do not mix with p+q outputs.
set -euo pipefail
source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/common.sh"

log_step "Purpose: compare quantized bottleneck-feature entropy of x+f(x) GAP mappings"
