"""Train fixed-epsilon p+q GAP mappings and test quantized encodings."""
...
def attack_labels(model, images: torch.Tensor, attack_goal: str, target: int) -> torch.Tensor:
    """Use the targeted or classic least-likely GAP training labels."""
...
@torch.no_grad()
def evaluate_mapping(model, mapping, data_loader, *, attack_goal: str, target: int, device: torch.device) -> dict:
    """Evaluate ASR and target-0 routing for one decoded mapping."""
...
def build_mapping(checkpoint: dict, *, gap_root: str, device: torch.device) -> FixedEpsilonPQMapping:
    """Rebuild the fixed-epsilon p+q mapping from checkpoint metadata."""
    ...
    mapping = FixedEpsilonPQMapping(generator, float(checkpoint["epsilon"])).to(device)
    mapping.load_state_dict(checkpoint["mapping"], strict=True)
    return mapping.eval()


def save_checkpoint(path: Path, mapping: FixedEpsilonPQMapping, metadata: dict) -> None:
...
def train_one_epsilon(
    *,
    ...
    """Train one fixed-epsilon generator, then evaluate all precisions."""
    ...
    base_metadata = {
        "protocol": "MDL-UAP-compression-v1",
        "mode": "imdep_pq_fixed",
        "side": side,
    ...
    for epoch in range(1, int(epochs) + 1):
        ...
        validation = evaluate_mapping(
            model, mapping, val_loader, attack_goal=attack_goal, target=target, device=device
        )
        record = {
            "epoch": epoch,
    ...
    for bits in PRECISIONS:
        ...
        validation = evaluate_mapping(
            model, decoded_mapping, val_loader, attack_goal=attack_goal, target=target, device=device
        )
        test = evaluate_mapping(
            model, decoded_mapping, test_loader, attack_goal=attack_goal, target=target, device=device
        )
    ...
    for item in rows:
        ...
        metadata_path = (
            output
            / "compression"
            / f"precision{item['quantization_bits']}"
            / "metadata.json"
        )
        metadata = json.loads(metadata_path.read_text(encoding="utf-8"))
        metadata_path.write_text(
            json.dumps({**metadata, **item}, indent=2), encoding="utf-8"
        )
    ...
    epsilon_summary = {
        **base_metadata,
        ...
        "minimum_precision_retaining_90": (
            int(minimum["quantization_bits"]) if minimum else None
        ),
...
def main() -> None:
    ...
    for side, result_path in (
        ("clean", args.clean_result),
        ("backdoor", args.backdoor_result),
    ):
    ...
    print(
        f"Run complete: conditions={len(epsilon_summaries)} encodings={len(all_rows)} "
